...
export async function sendEmail(data: IAttendanceSubmission) {
  ...
  const allSubmissionsHtml = `All submissions:<br/>
    <table border="1" cellpadding="10">
      ...
      ${rows.map((row, index) => `<tr><td>${++index}</td><td>${row.name}</td><td>${row.adults}</td><td>${row.kids}</td><td>${row.date}</td></tr>`).join('')}
    </table>
    <br/><br/>`;

  const sendTo = process.env.SENDGRID_SEND_TO
    ? process.env.SENDGRID_SEND_TO.split(',').map((emailAddress) => ({
